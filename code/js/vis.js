...
async function drawWorldMap() {
  console.log("script.js bottom reached");
  const mapElement = document.querySelector("#world-map");

  if (!mapElement) {
    console.log("No #world-map element found.");
    return;
  }
  ...
  console.log("First GeoJSON country properties:", world.features[0].properties);
  console.log("BRI status data loaded:", statusData);
  ...
  statusData.forEach(function (d) {
    ...
    if (text.includes("china")) {
      status = "china";
    } else if (text.includes("non-bri") || text.includes("control")) {
      status = "non_bri";
    } else if (text.includes("bri")) {
      status = "bri";
    }
  ...
  svg
    ...
    .attr("fill", function (d) {
      ...
      return colorScale(record.status);
    })
    .on("mouseover", function (event, d) {
      ...
      if (record) {
        if (record.status === "china") {
          statusText = "China";
        } else if (record.status === "bri") {
          statusText = "BRI country";
        } else if (record.status === "non_bri") {
          statusText = "Non-BRI country";
        }
      }

      tooltip
        .style("opacity", 1)
        .html(`<strong>${countryName}</strong><br>${statusText}`);
    })
    ...
    .on("click", function (event, d) {
      ...
      if (!iso3) {
        d3.select("#country-info").html(`
          ...
          No ISO3 code found for this country.
        `);
        return;
      }
      ...
      if (!record) {
        d3.select("#country-info").html(`
          <strong>${countryName}</strong><br>
          ISO3: ${iso3}<br>
          No BRI classification available.
        `);
        return;
      }
      ...
      if (record.status === "china") {
        ...
        statusLabel = "BRI country";
      } else if (record.status === "non_bri") {
        statusLabel = "Non-BRI country";
      }
      ...
      console.log("Clicked country:", record);

      // 以后你想让点击国家更新折线图，就在这里加：
      // updateLineChart(record.iso3);
    });
...
const radarIndicators = [
  ...
  },
  {
    ...
    column: "GNI per capita, Atlas method (current US$)",
  },
  {
    label: "GDP growth",
...
function toggleRadarYearAnimation(years) {
  ...
  // 如果当前已经是最后一年，再点 Play 就从第一年开始
  if (selectedRadarYear >= maxYear) {
    selectedRadarYear = minYear;
  }
...
async function drawRadarChart(iso3, countryName, keepPlaying = false) {
  selectedRadarCountry = { iso3: iso3, countryName: countryName };
  ...
  const years = Array.from(
    new Set(allData.map(d => +d["Year"]).filter(d => !isNaN(d)))
  ).sort((a, b) => a - b);

  if (!years.includes(selectedRadarYear)) {
    selectedRadarYear = d3.max(years);
  }
    ...
    <div class="radar-header">
      <div>
        ...
        <p class="radar-subtitle">
          Each axis is normalized to a 0–100 score relative to countries in the dataset.
        </p>
      </div>
      ...
      <div class="radar-year-control">
        ...
        <div class="radar-year-scale">
          <span>${d3.min(years)}</span>
          <span>${d3.max(years)}</span>
        </div>
      </div>
    </div>
  ...
  if (!countryRow) {
    d3.select("#radar-chart").html(`
      <p class="radar-empty">
        No WDI data available for ${countryName} in ${selectedRadarYear}.
      </p>
    `);
    return;
  }
  ...
  const radarValues = radarIndicators.map(indicator => {
    ...
    if (!isNaN(rawValue) && maxValue !== minValue) {
  ...
  for (let level = 1; level <= levels; level++) {
    ...
    const points = radarValues.map((d, i) => {
      ...
      return [
        rScale(levelValue) * Math.cos(angle),
        rScale(levelValue) * Math.sin(angle),
      ];
    });
